    ...
    (
        SELECT
            ...
            CASE
                ...
                THEN 'SWING COST'
                WHEN dp2.code_valeur IS NOT NULL
                THEN trim(dp2.lib_poste_l)
                ELSE trim(dp.lib_poste_l)
            END AS lib_poste ,
            CASE
                WHEN ct.forcage_cellule != ' '
                AND LENGTH(trim(TRANSLATE(ct.forcage_cellule,' +-.,1234567890',' '))) IS NULL
                THEN to_number(REPLACE(REPLACE(trim(ct.forcage_cellule),',',NULL),'.',','))
                WHEN ct.contenu_cellule IS NULL
                THEN 0
                ELSE to_number(REPLACE(REPLACE(trim(ct.contenu_cellule),',',NULL),'.',','))
            ...
            CASE
                ...
                WHEN pe.balise_fixfee = 'SWING_COST'
                AND ct.forcage_cellule = ' '
                THEN to_number(REPLACE(REPLACE(trim(ct.contenu_cellule),',',NULL),'.',','))
                WHEN dp2.code_valeur IS NOT NULL
                ...
                THEN to_number(REPLACE(REPLACE(trim(ctd3.contenu_cellule),',',NULL),'.',','))
                ELSE to_number(REPLACE(REPLACE(trim(ctd2.contenu_cellule),',',NULL),'.',','))
            END montant_poste ,
            CASE
        ...
        FROM
            parametrage_majcel pe
        LEFT JOIN
            contenu_tableau ct
        ON
            pe.type_calcul = ct.type_reporting
        ...
        LEFT JOIN
            justificatif_tableau jt
        ON
            pe.type_calcul = jt.type_reporting
        ...
        ON
            pe.type_calcul = ctd.type_reporting
        ...
        AND jt.numero_ligne_detail = ctd2.numero_ligne_detail
        AND ctd2.numero_colonne_detail =ct.numero_colonne +6 --- dans les tableaux details FEEPF et
            -- FEEPP le montant est sur a colonne de contenu_tableau +6 ( 15 =9+6 et 16= 10+6)
            -- Pour FEE-DP
        LEFT JOIN
        ...
        AND ctd3.numero_colonne_detail =ct.numero_colonne +3 -- dans les tableaux details FEEDP  le
            -- montant est sur a colonne du contenu_tableau +3   =9+6 et 16= 10+6
            -- poste la recuperation du code poste est critique. la colonne code poste devrait
            -- être non null
        LEFT JOIN
            descriptif_poste dp
        ON
        ...
        ON
            trim(REGEXP_SUBSTR(ctd.contenu_cellule,'\s\w+\s')) = trim(dp2.code_valeur) -- le code
            -- valeur est le premier groupe de caracteres delimite par des espaces
