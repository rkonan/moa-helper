...
-- Paramétrage métier des cumuls PRIIPS.
(
    SELECT
        cr.*,
        pcc.*,
    ...
    WHERE
        (
            (pcc.valeur_champ_ref IN ('OTHMOV', 'INIMOV') AND pcc.code_gestion = 'N')
            OR
            (pcc.valeur_champ_ref IN ('INIMOR', 'OTPA') AND pcc.code_gestion = 'O')
        )
        AND pcc.code_traitement = 'FCEL'
        AND cr.code_tableau = 'PRFEE3'
        AND cr.identifiant_cumul IN ('CHG_INT', 'CHG_OTH', 'ENT_OTH', 'ENT_INT')
),
...
(
    ...
    JOIN contexte_priips cp
        ON cp.type_reporting = pc1.type_reporting_cr
    ...
    LEFT JOIN tables_fixfee gt3
        ON gt3.code_portefeuille = cp.code_portefeuille
        AND gt3.date_arrete = cp.date_arrete
        AND gt3.famille_fixfee = 'FEE-DP'
        AND cp.code_part = gt3.code_part_fixfee
    LEFT JOIN tables_fixfee gt4
        ON gt4.code_portefeuille = cp.code_portefeuille
...
(
    SELECT
        ...
        CASE
            WHEN TRIM(db.libelle_balise) IS NULL THEN TRIM(db2.libelle_balise)
    ...
    JOIN parametrage_cumul pc
        ...
        AND pcc.critere_saisie_2 IN
            ('MGTF_EXA', 'DISF_EXA', 'DISF_EXP', 'NEG_INT', 'MGTF_EXP')
...
(
    ...
    LEFT JOIN contenu_tableau ct
        ...
        AND ct.code_tableau = pmc.choix_code_tableau
